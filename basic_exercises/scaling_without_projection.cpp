...
int main() 
{
    ...
    while(!glfwWindowShouldClose(mainWindow))
    {
        ...
        if(sizeOffset >= maxSize || sizeOffset <= minSize)
        {
        ...
        // The order of operation can change drastically the final result. Need to take into account how one trasformation will affect the other transformations.
        model = glm::translate(model, glm::vec3(triOffset, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(sizeOffset, sizeOffset, 1.0f));
