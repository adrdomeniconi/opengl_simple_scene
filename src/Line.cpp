...
void Line::Create(GLfloat *vertices, unsigned int numOfVertices)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numOfVertices * VERTEX_LENGTH, vertices, GL_STATIC_DRAW);  //Copy the data to the GPU
...
{
    glVertexAttribPointer(index, vertexAttributesLength, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * VERTEX_LENGTH, (void *)(sizeof(vertices[0]) * offset));
